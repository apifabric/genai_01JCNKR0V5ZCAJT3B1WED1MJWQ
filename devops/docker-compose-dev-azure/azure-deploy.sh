...
resourcename="holidayscheduler"  # lower case, only
resourcegroup="holidayscheduler_rg"
...
# git clone https://github.com/apilogicserver/holidayscheduler.git
# cd holidayscheduler
...
az group create --name holidayscheduler_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group holidayscheduler_rg --sku S1 --is-linux
az container create --resource-group holidayscheduler_rg --name holidayscheduler --image apilogicserver/holidayscheduler:latest --dns-name-label holidayscheduler --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//holidayscheduler.westus.azurecontainer.io:5656'
